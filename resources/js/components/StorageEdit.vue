   ...
   <div >
		<div class="modal fade" id="StorageModal" tabindex="-1" role="dialog" aria-labelledby="StorageModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <form id='formStorage' method="post" :action="url" @submit.prevent="validateBeforeSubmit">
...
export default {
    ...
    mounted() {
        ...
        $('#StorageModal').on('show.bs.modal',(event)=> {
            var button = $(event.relatedTarget) // Button that triggered the modal
            var storage = button.data('json') // Extract info from data-* attributes
            this.title ='Nuevo Almacen ';
            if(storage)
            {
                this.title='Editar '+storage.name;

                axios.get(`storage/${storage.id}`).then(response=>{
                        this.form = response.data.storage;
    ...
    methods:{
        validateBeforeSubmit() {
            this.$validator.validateAll().then((result) => {
                if (result) {
                let form = document.getElementById("formStorage");
